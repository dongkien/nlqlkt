## Slide 1

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-1-1.png "Image 0")

DTU301 · CHƯƠNG 4 · QLKT VĨ MÔ (KT MỞ)

Buổi 10

Mô hình Mundell-Fleming

IS-LM-BP + tỷ giá cố định/thả nổi

TS. Đỗ Ngọc Kiên  ·  Giảng viên phụ trách

Bộ môn Kinh tế và Quản lý --- Viện KT & KDQT

Năm học 2025--2026

## Slide 2

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-2-1.png "Image 0")

01

CHƯƠNG 4 · PHẦN MỘT

Khung Mundell-Fleming

## Slide 3

Trường Đại học Ngoại thương · SEIB

NỘI DUNG

Lộ trình Buổi 10

01

Khung MF

Lịch sử + ba phương trình

02

IS-LM-BP

Cân bằng ba thị trường

03

Tỷ giá cố định

Hiệu lực CSTK vs CSTT

04

Tỷ giá thả nổi + Cases

HK, Reagan, ERM

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

3

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-3-1.png "Image 0")

## Slide 4

Trường Đại học Ngoại thương · SEIB

PHẦN A · A.1

Vì sao cần Mundell-Fleming?

- IS-LM Hicks 1937: chỉ cho KT đóng.
- KT mở cần thêm dòng vốn quốc tế.
- Robert Mundell + J. Marcus Fleming 1960s.
- Nobel Mundell 1999 --- kiến trúc KT mở.
- Cơ sở phân tích chính sách hội nhập.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

4

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-4-1.png "Image 0")

## Slide 5

Trường Đại học Ngoại thương · SEIB

PHẦN A · A.2

Ba phương trình MF

- IS: Y = C(Y-T) + I(r) + G + NX(e).
- LM: M/P = L(Y, r).
- BP: NX(e) + KA(r - r\*) = 0.
- 3 biến nội sinh: Y, r, e.
- Cân bằng đồng thời 3 thị trường.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

5

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-5-1.png "Image 0")

## Slide 6

Trường Đại học Ngoại thương · SEIB

PHẦN A · A.3

Điều kiện Marshall-Lerner

- Điều kiện: \|ηx\| + \|ηm\| \> 1.
- Cần thiết cho NX tăng khi e tăng.
- Nếu không thoả → J-curve hiệu ứng.
- VN: phụ thuộc NK nhiều, ML có thể không thoả.
- SV: học khái niệm, không sâu derivation.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

6

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-6-1.png "Image 0")

## Slide 7

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-7-1.png "Image 0")

02

CHƯƠNG 4 · PHẦN HAI

IS-LM-BP cho KT mở

## Slide 8

Trường Đại học Ngoại thương · SEIB

PHẦN B · B.1

Đường IS mở rộng

- IS mở: thêm NX(e, Y).
- e tăng → NX tăng (Marshall-Lerner).
- Y tăng → NX giảm (NK tăng theo Y).
- IS mở dốc hơn IS đóng.
- Có thể dịch do G, T, hoặc e tự định.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

8

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-8-1.png "Image 0")

## Slide 9

Trường Đại học Ngoại thương · SEIB

PHẦN B · B.2

Đường LM mở rộng

- LM: tương tự đóng (M/P = L(Y,r)).
- Khác: M có thể thay đổi do can thiệp NHTW.
- Tỷ giá cố định: M nội sinh.
- Tỷ giá thả nổi: M độc lập.
- Đây là điểm khác biệt then chốt.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

9

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-9-1.png "Image 0")

## Slide 10

Trường Đại học Ngoại thương · SEIB

PHẦN B · B.3

Đường BP

- BP: cán cân thanh toán = 0.
- Phụ thuộc độ luân chuyển vốn κ.
- κ ≈ 0: BP nghiêng (vốn hạn chế).
- κ ≈ ∞: BP nằm ngang (vốn tự do).
- Mankiw giả định κ ≈ ∞ (đơn giản hoá).

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

10

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-10-1.png "Image 0")

## Slide 11

Trường Đại học Ngoại thương · SEIB

PHẦN B · B.4

Cân bằng IS-LM-BP

- 3 đường giao tại 1 điểm: cân bằng đồng thời.
- Cân bằng nội: IS ∩ LM.
- Cân bằng ngoại: BP.
- Cùng cân bằng: 3 đường cùng giao.
- Cơ chế tự động đưa về cân bằng.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

11

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-11-1.png "Image 0")

## Slide 12

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-12-1.png "Image 0")

03

CHƯƠNG 4 · PHẦN BA

Tỷ giá cố định

## Slide 13

Trường Đại học Ngoại thương · SEIB

PHẦN C · C.1

Tỷ giá cố định + CSTK

- G tăng → IS dịch phải.
- Y\* tăng, r tăng tạm thời.
- Vốn vào → áp lực e giảm.
- NHTW mua \$ → M tăng → LM dịch phải.
- Cuối cùng: Y tăng MẠNH, r về cũ.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

13

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-13-1.png "Image 0")

## Slide 14

Trường Đại học Ngoại thương · SEIB

PHẦN C · C.2

Tỷ giá cố định + CSTT

- M tăng → LM dịch phải tạm thời.
- r giảm → vốn ra.
- Áp lực e tăng.
- NHTW bán \$ → M giảm về cũ.
- CSTT MẤT TÁC DỤNG dưới peg.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

14

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-14-1.png "Image 0")

## Slide 15

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-15-1.png "Image 0")

04

CHƯƠNG 4 · PHẦN BỐN

Tỷ giá thả nổi + Cases

## Slide 16

Trường Đại học Ngoại thương · SEIB

PHẦN D · D.1

Tỷ giá thả nổi + CSTT

- M tăng → LM dịch phải.
- r giảm → vốn ra → e tăng.
- e tăng → NX tăng → Y tăng thêm.
- Hai kênh: r và e.
- CSTT MẠNH dưới thả nổi.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

16

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-16-1.png "Image 0")

## Slide 17

Trường Đại học Ngoại thương · SEIB

PHẦN D · D.2

Tỷ giá thả nổi + CSTK

- G tăng → IS dịch phải tạm thời.
- r tăng → vốn vào → e giảm.
- e giảm → NX giảm → IS dịch ngược.
- Cuối cùng: Y không đổi (về cũ).
- CSTK BỊ TRIỆT TIÊU qua tỷ giá.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

17

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-17-1.png "Image 0")

## Slide 18

Trường Đại học Ngoại thương · SEIB

PHẦN D · D.3

Case Hong Kong LERS

- Linked Exchange Rate System từ 1983.
- HKD-USD peg 7.80 (sau 7.75-7.85 từ 2005).
- Currency board chuẩn.
- 2022-2023: Fed tăng r 525 bps.
- HK tự động nâng r theo Fed (chứng minh C.2).

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

18

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-18-1.png "Image 0")

## Slide 19

Trường Đại học Ngoại thương · SEIB

PHẦN D · D.4

Reaganomics và ERM 1992

- Reagan 1981-1985: G tăng + cắt thuế.
- USD tăng giá mạnh, NX giảm.
- Twin deficit (ngân sách + thương mại).
- ERM 1992: GBP rời ERM (Soros).
- Bài học: tỷ giá cố định khó duy trì.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

19

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-19-1.png "Image 0")

## Slide 20

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-20-1.png "Image 0")

★

CHƯƠNG 4 · PHẦN KẾT

Tổng kết và Bài tập về nhà

## Slide 21

Trường Đại học Ngoại thương · SEIB

TỔNG KẾT BUỔI 10

Năm điểm cốt lõi

- 1. MF: IS-LM-BP cho KT mở.
- 2. Tỷ giá cố định: CSTK mạnh, CSTT vô hiệu.
- 3. Tỷ giá thả nổi: CSTT mạnh, CSTK triệt tiêu.
- 4. HK LERS: bằng chứng cố định.
- 5. ERM 1992: bài học cố định khó duy trì.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

21

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-21-1.png "Image 0")

## Slide 22

Trường Đại học Ngoại thương · SEIB

BÀI TẬP VỀ NHÀ

Chuẩn bị Buổi 11

- Đọc về Bộ ba bất khả thi (Mundell 1963).
- Tự đọc tổng quan Khủng hoảng Á 1997.
- Chuẩn bị 1 ví dụ VN trong Trilemma.
- Đọc thêm về sterilization của TQ.

DTU301 · Buổi 10 · Chương 4 --- QLKT vĩ mô (KT mở)

22

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-22-1.png "Image 0")

## Slide 23

![preencoded.png](/home/kiendn/material-building/_source-materials/slides/media-10/ppt/media/image-23-1.png "Image 0")

Cảm ơn --- chuyển sang Buổi 11

TS. Đỗ Ngọc Kiên  ·  Giảng viên phụ trách

Bộ môn Kinh tế và Quản lý · SEIB

Buổi 11 --- Trilemma và Khủng hoảng Á 1997 (vẫn Ch.4)
