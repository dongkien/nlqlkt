## Slide 1

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-1-1.png "Image 0")

DTU301 · SEMINAR · BUỔI 15

Buổi 15

Thuyết trình + Tổng kết

Kết thúc học phần Nguyên lý QLKT

TS. Đỗ Ngọc Kiên  ·  Giảng viên phụ trách

Bộ môn Kinh tế và Quản lý --- Viện KT & KDQT

Năm học 2025--2026

## Slide 2

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-2-1.png "Image 0")

01

SEMINAR · PHẦN MỘT

Tổng hợp 5 chương

## Slide 3

Trường Đại học Ngoại thương · SEIB

NỘI DUNG

Lộ trình Buổi 15

01

Tổng hợp 5 chương

Liên kết khung lý thuyết

02

Trình bày nhóm

Thứ tự + quy tắc

03

Tiêu chí đánh giá

Nội dung + trình bày + nhóm

04

Bài học cuối khoá

5 thông điệp + kỹ năng

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

3

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-3-1.png "Image 0")

## Slide 4

Trường Đại học Ngoại thương · SEIB

PHẦN A · A.1

Tổng hợp 5 chương

- Ch.1: Tổng quan QLKT (Bentham, Kant, Rawls).
- Ch.2: QLKT ngành + thất bại thị trường.
- Ch.3: QLKT vĩ mô KT đóng (IS-LM, AD-AS).
- Ch.4: QLKT vĩ mô KT mở (MF, Trilemma).
- Ch.5: Quản trị địa phương + vùng.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

4

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-4-1.png "Image 0")

## Slide 5

Trường Đại học Ngoại thương · SEIB

PHẦN A · A.2

Bốn cấp độ QLKT

- 1. Ngành: cấu trúc TT, điều tiết.
- 2. Vĩ mô đóng: tài khoá, tiền tệ.
- 3. KT mở: BoP, tỷ giá, MF.
- 4. Địa phương + vùng: phân cấp, NEG.
- Mỗi cấp độ có công cụ + lý thuyết riêng.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

5

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-5-1.png "Image 0")

## Slide 6

Trường Đại học Ngoại thương · SEIB

PHẦN A · A.3

Khung lý thuyết cốt lõi

- Vi mô: cung cầu, cấu trúc TT, ngoại tác.
- Vĩ mô: IS-LM, AD-AS, Phillips.
- Mở: Mundell-Fleming, Trilemma.
- Phát triển: Solow + Cobb-Douglas.
- Vùng: NEG Krugman.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

6

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-6-1.png "Image 0")

## Slide 7

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-7-1.png "Image 0")

02

SEMINAR · PHẦN HAI

Trình bày của các nhóm

## Slide 8

Trường Đại học Ngoại thương · SEIB

PHẦN B · B.1

Thứ tự trình bày

- Bốc thăm thứ tự đầu buổi.
- Mỗi nhóm 15-20 phút + 10 phút Q&A.
- Tổng cộng 4-6 nhóm trong 3 giờ.
- Nghỉ giữa giờ 10 phút.
- Cuối buổi: tổng kết của GV.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

8

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-8-1.png "Image 0")

## Slide 9

Trường Đại học Ngoại thương · SEIB

PHẦN B · B.2

Quy tắc trình bày

- Đúng thời gian (cảnh báo 2 phút cuối).
- Phân công rõ thành viên.
- Slide rõ + đầy đủ trích dẫn.
- Trình bày không đọc slide.
- Tương tác với khán giả.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

9

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-9-1.png "Image 0")

## Slide 10

Trường Đại học Ngoại thương · SEIB

PHẦN B · B.3

Phản biện chéo nhóm

- Mỗi nhóm phản biện 1 nhóm khác (bốc thăm).
- Phản biện 5 phút: 3 mạnh + 3 yếu.
- Nhóm trả lời 3 phút.
- GV tổng hợp + góp ý.
- Đo lường CLO3 + CLO4.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

10

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-10-1.png "Image 0")

## Slide 11

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-11-1.png "Image 0")

03

SEMINAR · PHẦN BA

Tiêu chí đánh giá

## Slide 12

Trường Đại học Ngoại thương · SEIB

PHẦN C · C.1

Tiêu chí nội dung

- Đúng khung lý thuyết + dữ liệu.
- Phân tích sâu, có đánh đổi.
- Đề xuất khả thi cho VN.
- Sử dụng dữ liệu thực tế (cập nhật).
- Trích dẫn đầy đủ (APA 7th).

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

12

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-12-1.png "Image 0")

## Slide 13

Trường Đại học Ngoại thương · SEIB

PHẦN C · C.2

Tiêu chí trình bày

- Slide thiết kế chuyên nghiệp.
- Trình bày rõ ràng, lưu loát.
- Tương tác với khán giả.
- Trả lời câu hỏi tốt.
- Quản lý thời gian.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

13

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-13-1.png "Image 0")

## Slide 14

Trường Đại học Ngoại thương · SEIB

PHẦN C · C.3

Tiêu chí phối hợp

- Phân công công việc rõ ràng.
- Đóng góp đồng đều.
- Peer evaluation: SV tự chấm nhau.
- Bản khai sử dụng AI có trách nhiệm.
- Tinh thần học thuật.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

14

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-14-1.png "Image 0")

## Slide 15

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-15-1.png "Image 0")

04

SEMINAR · PHẦN BỐN

Bài học cuối khoá

## Slide 16

Trường Đại học Ngoại thương · SEIB

PHẦN D · D.1

Năm thông điệp lớn

- 1. QLKT = phương thức XH trả lời 3 câu hỏi.
- 2. Thị trường + Nhà nước, không loại trừ.
- 3. Mọi chính sách có đánh đổi và rủi ro.
- 4. Hội nhập + tự chủ là cân bằng tinh tế.
- 5. Phân cấp đi cùng năng lực địa phương.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

16

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-16-1.png "Image 0")

## Slide 17

Trường Đại học Ngoại thương · SEIB

PHẦN D · D.2

Kỹ năng phân tích chính sách

- 1. Phân biệt thực chứng vs chuẩn tắc.
- 2. Dùng khung lý thuyết phù hợp.
- 3. Tìm dữ liệu chất lượng.
- 4. Đánh giá đánh đổi và rủi ro.
- 5. Đề xuất khả thi và minh bạch.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

17

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-17-1.png "Image 0")

## Slide 18

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-18-1.png "Image 0")

★

SEMINAR · PHẦN KẾT

Kết thúc học phần

## Slide 19

Trường Đại học Ngoại thương · SEIB

LỜI CẢM ƠN

Bước tiếp theo cho sinh viên

- Cảm ơn các bạn đã hoàn thành học phần.
- Kết nối với học phần chuyên sâu (KTĐN).
- Theo dõi cập nhật chính sách thường xuyên.
- Tham gia hoạt động nghiên cứu (NCKH).
- Chúc thành công trên hành trình KT.

DTU301 · Buổi 15 · Seminar tổng hợp và ứng dụng

19

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-19-1.png "Image 0")

## Slide 20

![preencoded.png](/home/kiendn/nlqlkt/_source-materials/slides/media-15/ppt/media/image-20-1.png "Image 0")

Cảm ơn --- Kết thúc học phần

TS. Đỗ Ngọc Kiên  ·  Giảng viên phụ trách

Bộ môn Kinh tế và Quản lý · SEIB

DTU301 --- Nguyên lý Quản lý Kinh tế · Năm học 2025--2026
